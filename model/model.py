...
class Model:
    def __init__(self):
        ...
        self.nlp1 = pipeline("sentiment-analysis", model="tarnformnet/Stock-Sentiment-BERT",
    ...
    def predict(self, dataframe_path: str, output_path: str):
        ...
        # Using map function
        df["sentiment"] = df["TweetText"].map(
            lambda x: self.nlp1(x)[0]['label'])
        df["sentiment_base"] = df["TweetText"].map(
            lambda x: self.nlp2(x)[0]['label'])
