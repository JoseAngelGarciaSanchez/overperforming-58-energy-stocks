...
# Layout
st.set_page_config(
    page_title="Macro - Cross Chain Monitoring", page_icon=":bar_chart:", layout="wide"
)
st.title("🌍 Macro KPIs")
...
def get_data(query):
    if query == "Transactions Overview":
        return pd.read_json(
            "https://node-api.flipsidecrypto.com/api/v2/queries/579714e6-986e-421a-85dd-c32a8b41b25c/data/latest"
        )
    elif query == "Transactions Daily":
        return pd.read_json(
            "https://node-api.flipsidecrypto.com/api/v2/queries/4e0c69ff-9395-43c1-af49-f590f864d339/data/latest"
        )
    elif query == "Transactions Heatmap":
        return pd.read_json(
            "https://node-api.flipsidecrypto.com/api/v2/queries/9d8d54d4-b700-4d85-af17-8c29aa29d334/data/latest"
        )
    elif query == "Transactions Fee Payers":
        return pd.read_json(
            "https://node-api.flipsidecrypto.com/api/v2/queries/7eae69ea-2387-420d-b4b9-6eceeb5ef22d/data/latest"
        )
    elif query == "Transactions New Users":
        return pd.read_json(
            "https://node-api.flipsidecrypto.com/api/v2/queries/d81c5861-0792-43ec-9f92-d89fbcf85e79/data/latest"
        )
    return None
...
# Single chain Analysis
    ...
    with c1:
        st.metric(
            label="**Total Blocks**",
            value=str(df["Blocks"].map("{:,.0f}".format).values[0]),
        )
        st.metric(
            label="**Average Transactions/Block**",
            value=str(df["Transactions/Block"].map("{:,.0f}".format).values[0]),
        )
    with c2:
        st.metric(
            label="**Total Transactions**",
            value=str(df["Transactions"].map("{:,.0f}".format).values[0]),
        )
        st.metric(
            label="**Average TPS**",
            value=str(df["TPS"].map("{:,.2f}".format).values[0]),
        )
    with c3:
        st.metric(
            label="**Total Unique Addresses**",
            value=str(df["Users"].map("{:,.0f}".format).values[0]),
        )
        st.metric(
            label="**Average Daily Active Users**",
            value=str(df["Users/Day"].map("{:,.0f}".format).values[0]),
        )
    ...
    fig.add_trace(
        go.Line(x=df["Date"], y=df["Blocks"], name="Blocks"), secondary_y=True
    )
    ...
    fig.add_trace(
        go.Line(x=df["Date"], y=df["Transactions/Block"], name="Transactions/Block"),
        secondary_y=True,
    )
    ...
    fig = px.density_heatmap(
        ...
        z="Users",
        histfunc="avg",
        title="Heatmap of Active Addresses",
        nbinsx=24,
    )
    fig.update_layout(
        legend_title=None,
    ...
    fig.update_layout(
        ...
        yaxis_title=None,
        xaxis={"dtick": 1},
        coloraxis_colorbar=dict(title="Blocks"),
    )
    fig.update_yaxes(categoryorder="array", categoryarray=week_days)
    st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
...
# Cross Chain Comparison
    ...
    fig = px.line(
        df, x="Date", y="Blocks", color="Blockchain", title="Daily Blocks", log_y=True
    )
    ...
    with c1:
        df = transactions_heatmap.query("Blockchain == @options")
        df["Transactions"] = df.groupby("Blockchain")["Transactions"].transform(
        ...
        fig = px.density_heatmap(
            ...
            z="Transactions",
            histfunc="avg",
            title="Daily Heatmap of Normalized Transactions",
        )
        fig.update_layout(
            legend_title=None,
            xaxis_title=None,
            yaxis_title=None,
            coloraxis_colorbar=dict(title="Min/Max"),
        )
        ...
        df = transactions_heatmap.query("Blockchain == @options")
        df["Users"] = df.groupby("Blockchain")["Users"].transform(
            lambda x: (x - x.min()) / (x.max() - x.min())
        )
        fig = px.density_heatmap(
            df,
            ...
            z="Users",
            histfunc="avg",
            title="Daily Heatmap of Normalized Users",
        )
        fig.update_layout(
            legend_title=None,
            xaxis_title=None,
        ...
        df = transactions_heatmap.query("Blockchain == @options")
        df["Blocks"] = df.groupby("Blockchain")["Blocks"].transform(
            lambda x: (x - x.min()) / (x.max() - x.min())
        )
        fig = px.density_heatmap(
            df,
            ...
            z="Blocks",
            histfunc="avg",
            title="Daily Heatmap of Normalized Blocks",
        )
        fig.update_layout(
            legend_title=None,
            xaxis_title=None,
    ...
    with c2:
        ...
        fig = px.density_heatmap(
            ...
            z="Transactions",
            histfunc="avg",
            title="Hourly Heatmap of Normalized Transactions",
            nbinsy=24,
        )
        fig.update_layout(
            legend_title=None,
        ...
        fig = px.density_heatmap(
            ...
            z="Users",
            histfunc="avg",
            title="Hourly Heatmap of Normalized Users",
            nbinsy=24,
        )
        fig.update_layout(
            legend_title=None,
        ...
        fig = px.density_heatmap(
            ...
            z="Blocks",
            histfunc="avg",
            title="Hourly Heatmap of Normalized Blocks",
            nbinsy=24,
        )
        fig.update_layout(
            legend_title=None,
